...
command+Shift+A：选择当前标签前后，修改标签用的
...
鼠标的前进后退键可切换Tab文件
按 command，依次点击或选取，可需要编辑的多个位置
按 command+Shift+上下键，可替换行
...
command+N：新建窗口
command+K+B：开关侧栏
command+Shift+M：选中当前括号内容，重复可选着括号本身
command+F2：设置/删除标记
```
...
command+Shift+↓ 将光标所在行和下一行代码互换（将光标所在行插入到下一行之后）。
